...
combined_mask_1x_path = fr"{base_path}/{file_number}_HU_combined_1x.nrrd"
...
combined_mask_2x_path = fr"{base_path}/{file_number}_HU_combined_2x.nrrd"
...
# def save_as_nifti(mask_array, output_path):
#     # Create a NIfTI image from the numpy array
#     nifti_image = nib.Nifti1Image(mask_array, np.eye(4))  # Identity matrix as placeholder for affine
#     nib.save(nifti_image, output_path)

# # Save combined masks as NIfTI
# combined_mask_1x_nii_path = fr"{base_path}/{file_number}_HU_combined_1x.nii"
# combined_mask_2x_nii_path = fr"{base_path}/{file_number}_HU_combined_2x.nii"
# save_as_nifti(combined_mask_1x, combined_mask_1x_nii_path)
# save_as_nifti(combined_mask_2x, combined_mask_2x_nii_path)

# print(f"合并后的 1x 遮罩已保存为 NIfTI 格式: {combined_mask_1x_nii_path}")
# print(f"合并后的 2x 遮罩已保存为 NIfTI 格式: {combined_mask_2x_nii_path}")

# # 输出每个模型的脂肪体积，先1X再2X
# print("\n--- 各模型脂肪体积统计 ---")
# for model_name, volume in model_volumes.items():
#     print(f"{model_name}: {volume:.2f} cm³")
